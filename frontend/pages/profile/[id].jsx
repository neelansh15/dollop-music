...
function profileId() {
    ...
    if (isFollowed) {
      try {
        const { data, status } = await axios.post(
          `${apiUrl}/api/follow/remove_following/`,
          usersData
        );
      ...
      try {
        const { data, status } = await axios.post(
          `${apiUrl}/api/follow/add_following/`,
          usersData
        );
        if (!data || status != 200) return;
        setFollowed(true);
  ...
  useEffect(async () => {
    ...
    const { data: email } = await axios.get(
      `${apiUrl}/api/profile/email/` + id.toLowerCase()
    );

    // Get User doc
    const { data: userDoc } = await axios.get(`${apiUrl}/api/profile/` + email);
    setUser(userDoc);
